export default function OurPrinciples() {
  ...
  return (
    <section className="px-6 max-w-[1400px] mx-auto w-full font-sans mb-20">
      <div className="bg-[#f4f8fb] rounded-xl p-10 md:p-16 lg:p-20 shadow-sm border border-gray-100">
        <h2 className="text-3xl md:text-4xl font-bold text-[#0f172a] text-center mb-20">
          Our Principles
        </h2>

        <div className="relative flex flex-col md:flex-row justify-between items-start max-w-6xl mx-auto gap-10 md:gap-4">
          
          {principles.map((p, idx) => (
            <div key={idx} className="flex-1 relative flex flex-col items-center text-center px-2">
              <div className="flex items-center w-full justify-center mb-6">
                <span className="text-3xl font-bold text-[#0f172a]">{p.num}</span>
                {idx < principles.length - 1 && (
                  <div className="hidden md:block absolute top-4 left-[60%] w-[80%] h-px bg-gray-300 z-0"></div>
                )}
              </div>
              <h4 className="text-base font-bold text-[#0f172a] mb-3">{p.title}</h4>
              <p className="text-gray-500 text-xs leading-relaxed max-w-[150px] mx-auto">
                {p.desc}
              </p>
            </div>
          ))}
