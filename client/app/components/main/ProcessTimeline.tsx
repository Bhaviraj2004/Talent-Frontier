  ...
  return (
    <section className="py-16 md:py-24 px-6 max-w-[1200px] mx-auto w-full font-sans text-center">
      ...
      <div className="relative flex flex-col md:flex-row justify-between items-center md:items-start w-full">
        ...
        {steps.map((step, idx) => (
          <div key={idx} className="flex flex-col items-center w-full md:w-auto mb-10 md:mb-0 relative z-10">
            <div className="w-14 h-14 rounded-full bg-[#2563eb] text-white flex items-center justify-center font-bold text-xl mb-4 md:mb-6 shadow-md md:outline md:outline-8 md:outline-white ring-4 ring-white">
              {idx + 1}
            </div>
            <p className="font-semibold text-gray-800 text-sm whitespace-pre-line px-2 bg-white md:bg-transparent rounded-lg py-1">{step}</p>
          </div>
        ))}
      </div>
    </section>
